...
export default function AIDiagnosis() {
  ...
  const handleDiagnosis = async () => {
    ...
    try {
      ...
      const apiURL = `https://api.wolframalpha.com/v2/query?input=${query}&format=plaintext&appid=${appId}`;
      ...
      if (
        ...
        response.data.queryresult.success
      ) {
        ...
        setDiagnosis(
          "Unable to fetch diagnosis. Please check your input and try again."
        );
      }
    } catch (error) {
      console.error("Error fetching diagnosis:", error); // Log any errors
      setDiagnosis(
        "An error occurred while fetching the diagnosis. Please try again."
      );
    } finally {
      setLoading(false);
    }
